...
from backend.app.database import engine, Base, async_session_factory
...
class TestDatabaseRecovery:
    ...
    async def test_schedule_pdf_route_recovers_missing_tables_without_retry_prompt(
        self,
        client: AsyncClient,
    ):
        async with engine.begin() as conn:
    ...
    async def test_schedule_generate_recovers_missing_tables_without_retry_prompt(
        self,
        client: AsyncClient,
    ):
        user_resp = await client.post("/api/users", json={
            "name": "Schedule Recovery User",
            "email": "schedule-recovery@example.com",
            "daily_study_hours": 3,
        })
        assert user_resp.status_code == 201
        user = user_resp.json()

        subject_resp = await client.post("/api/subjects", json={
            "user_id": user["id"],
        ...
        recovery_user_resp = await client.post("/api/users", json={
            "name": "Schedule Recovery User 2",
            "email": "schedule-recovery-2@example.com",
            "daily_study_hours": 3,
        })
        assert recovery_user_resp.status_code == 201
        recovery_user = recovery_user_resp.json()

        recovery_subject_resp = await client.post("/api/subjects", json={
            "user_id": recovery_user["id"],
            "name": "Operating Systems",
            "exam_date": (date.today() + timedelta(days=7)).isoformat(),
            "priority": 4.0,
        })
        assert recovery_subject_resp.status_code == 201
        recovery_subject = recovery_subject_resp.json()

        recovery_topic_resp = await client.post("/api/topics", json={
            "subject_id": recovery_subject["id"],
            "name": "Unit 1: Processes",
            "difficulty": 0.7,
            "estimated_hours": 2.0,
        })
        assert recovery_topic_resp.status_code == 201
...
class TestScheduleBehavior:
    async def test_get_schedule_auto_reschedules_overdue_pending_entries(self, client: AsyncClient):
        user = (await client.post("/api/users", json={
            "name": "Sched User", "email": "sched-user@example.com", "daily_study_hours": 3
        })).json()
        subject = (await client.post("/api/subjects", json={
            "user_id": user["id"],
        ...
        topic = (await client.post("/api/topics", json={
            "subject_id": subject["id"],
            "name": "Unit 1: Mathematical Logic",
            "difficulty": 0.7,
            "estimated_hours": 2.0,
        })).json()
        ...
        assert payload
        assert all(
            entry["completed"] == 1 or entry["scheduled_date"] >= date.today().isoformat()
            for entry in payload
        )

    async def test_unread_endpoint_marks_entry_unread(self, client: AsyncClient):
        user = (await client.post("/api/users", json={
            "name": "Unread User", "email": "unread-user@example.com", "daily_study_hours": 3
        })).json()
        subject = (await client.post("/api/subjects", json={
            "user_id": user["id"],
        ...
        resp = await client.patch(f"/api/schedule/unread/{entry_id}")
        assert resp.status_code == 200

        schedule_resp = await client.get(f"/api/schedule/{user['id']}")
        payload = schedule_resp.json()
        target = next(item for item in payload if item["id"] == entry_id)
